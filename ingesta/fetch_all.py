"""Etapa de descarga, por ambiente. Lanzalo y hace otra cosa mientras corre.
  ...
  python -m ingesta.fetch_all --ambiente bano
  python -m ingesta.fetch_all --ambiente cocina
  python -m ingesta.fetch_all --ambiente todos
  python -m ingesta.fetch_all --ambiente cocina --force   # refresca precios
  python -m ingesta.fetch_all --ambiente cocina --estimar # solo cuenta, cero red

DELAY minimo 1.8s por request (ver ingesta/fetch.py). Con --estimar se imprime
...
from config.categorias import (AMBIENTE_A_CATEGORIAS, categorias_de_ambiente,
                              guias_de_ambiente)
from ingesta.fetch import DELAY, fetch, url_categoria
from ingesta.parse import descubrir_facetas
...
def main() -> None:
    ...
    print(f"\n{n} paginas en cache. Ahora: python -m ingesta.parse_all")
