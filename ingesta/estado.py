...
from config.categorias import AMBIENTE_A_CATEGORIAS, PRODUCTOS, SOLO_GUIA
from ingesta.fetch import CACHE, MANIFEST, clave, manifest, url_categoria
from ingesta.parse import descubrir_facetas, leer_html
...
def main() -> int:
    ...
    print(f"\n  URLs pendientes: {pendientes}"
          + ("  -> corre: python -m ingesta.fetch_all --ambiente <bano|cocina|habitacion|sala|todos>"
    ...
    for ruta, desc, comando in (
            ("datos/productos.json", "productos parseados", "python -m ingesta.parse_all"),
            ("datos/guias.json", "corpus RAG", "python -m ingesta.parse_all")):
        p = pathlib.Path(ruta)
        if p.exists():
            try:
    ...
    print("=" * 70)
    db = pathlib.Path("datos/catalogo.db")
    if not db.exists():
        print("  datos/catalogo.db FALTA -> python -m ingesta.build_index")
        return 1
    ...
    con.close()
    print(f"  datos/catalogo.db: {n_p} productos en {cats} categorias, {n_g} chunks de guia")
    print(f"  snapshot: {fecha}")
    if fixture:
        print(f"  OJO: {fixture} de esos productos son del FIXTURE sintetico.")
        print("       Corre: python -m ingesta.build_index   (sin --fuente)")
    if n_g == 0:
        print("  OJO: 0 chunks de guia -> consultar_guia no tiene fuente y NO se")
        print("       alcanza el nivel 4 de la rubrica. Revisa datos/guias.json.")
    return 0
