...
por buenos, y abre a mano los 3 links que imprime al final.
  ...
  python -m ingesta.sanity                     # todo datos/productos.json
  python -m ingesta.sanity datos/productos.json bano
  python -m ingesta.sanity datos/productos.json cocina
"""
from __future__ import annotations
import json, pathlib, statistics as st, sys
from config.categorias import AMBIENTE_A_CATEGORIAS


def main(ruta: str = "datos/productos.json", ambiente: str | None = None) -> int:
    p = pathlib.Path(ruta)
    if not p.exists():
