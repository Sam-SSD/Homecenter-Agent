...
def main() -> None:
    ...
    ap.add_argument("--fuente", default="data/productos.json")
    ap.add_argument("--guias", default="data/guias.json")
    a = ap.parse_args()

    ruta = pathlib.Path(a.fuente)
    if not ruta.exists():
        print(f"no existe {ruta}. Opciones:")
        print("  python -m ingest.fetch_all --etapa 1 && python -m ingest.parse_all")
